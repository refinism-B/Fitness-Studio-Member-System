...
def get_member_stock(member_id: str, plan: str):
    """取得特定會員特定方案的庫存狀況"""
    try:
...
def validate_refund(member_id: str, plan: str, coach_name: str) -> tuple[bool, str, dict]:
    """
    驗證退款資料
    Args:
        member_id: 會員編號
    ...
    try:
        ...
        result = get_member_stock(member_id, plan)
        ...
        coach_id, _ = gr.get_coach_id(coach=coach_name)
...
def execute_refund(data: dict) -> tuple[bool, str]:
    """
    執行退款寫入
    """
    try:
